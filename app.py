...
st.set_page_config(page_title="Ismaily SC - Final Stability", layout="wide")
st.title("⚽ كشاف الإسماعيلي: النسخة المستقرة والمصححة")
...
if uploaded_file:
    ...
    for match in player_pattern.finditer(data):
        ...
        if start_offset + 300 <= len(data):
            ...
            # 2. البحث عن المهارات بشكل مستقل (مع تصحيح الترحيل للأمام)
            pace, stamina, strength = 0, 0, 0
            for j in range(100, 200):
                if j + 3 < len(record):
                    # فحص بلوك المهارات (3 أرقام منطقية)
    ...
    if results:
        ...
        st.success(f"✅ تم التشغيل بنجاح! وجدنا {len(df)} لاعب.")
